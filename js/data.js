/*
 * RC — Gümüş fiyat verisi (XAGUSDT perpetual).
 * Sırasıyla Binance, Bybit ve OKX denenir; hepsi anahtarsız ve CORS destekli.
 * TradingView grafiğinde aynı kontrat gösterilir: BINANCE:XAGUSDT.P
 */
(function (root) {
  'use strict';
  ...
  var PROVIDERS = [
    {
      id: 'binance', name: 'Binance · XAGUSDT',
      load: function (tf) {
        return fetchJson('https://fapi.binance.com/fapi/v1/klines?symbol=XAGUSDT&interval=' + TIMEFRAMES[tf].binance + '&limit=600')
          .then(function (rows) { return rows.map(function (k) { return row(k[0], k[1], k[2], k[3], k[4]); }); });
      }
    },
    {
      id: 'bybit', name: 'Bybit · XAGUSDT',
      load: function (tf) {
        return fetchJson('https://api.bybit.com/v5/market/kline?category=linear&symbol=XAGUSDT&interval=' + TIMEFRAMES[tf].bybit + '&limit=600')
          .then(function (j) {
    ...
    {
      id: 'okx', name: 'OKX · XAG-USDT-SWAP',
      load: function (tf) {
        return fetchJson('https://www.okx.com/api/v5/market/candles?instId=XAG-USDT-SWAP&bar=' + TIMEFRAMES[tf].okx + '&limit=300')
          .then(function (j) {
  ...
  /** { candles, source } — son mum henüz kapanmamış olabilir. */
  function loadSilver(tf) {
  ...
  var api = { TIMEFRAMES: TIMEFRAMES, PROVIDERS: PROVIDERS, loadSilver: loadSilver, closedOnly: closedOnly };
