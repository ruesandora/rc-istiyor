...
// Zamanlayıcı gecikse de kaçmasın diye en az 6 saat (ve en az 2 mum) geriye bakılır
const CONFIRM_WINDOW = 2;
function confirmWindow(tf) { return Math.max(CONFIRM_WINDOW, Math.ceil(6 * 3600 / TF[tf].seconds)); }

/** Bir zaman dilimi için bildirilecek olaylar. */
function evaluate(candles, tf, opts, nowSec) {
  const closed = API.closedOnly(candles, tf, nowSec);
  const res = D.findDivergences(closed, opts);
  const last = closed.length - 1;
  const confirmed = res.divergences
    .filter(d => d.confirmIndex >= last - confirmWindow(tf))
...
function dedupeKey(ev, tf, asset) {
  asset = asset || 'silver';
  // Olası: aynı çıpa pivot + tür için tek bildirim (aday kaysa bile tekrar etmez)
  if (ev.kind === 'potential') return `rc:pot:${asset}:${tf}:${ev.type}:${ev.from.time}`;
  return `rc:div:${asset}:${tf}:${ev.type}:${ev.from.time}:${ev.to.time}`;
}
...
function n1(v) { return String(Math.round(v)); } // RSI tam sayı: fiyatla karışmasın
function date(t, tf) {
...
function formatMessage(ev, tf, siteUrl, asset) {
  const name = (API.ASSETS[asset] || API.ASSETS.silver).name;
  ...
  if (siteUrl) lines.push('', `<a href="${esc(siteUrl + (asset === 'gold' ? '#altin' : ''))}">Grafiği aç →</a>`);
...
function store(env) {
  ...
  return {
    /** Genel Redis komutu: cmd('SADD', 'k', 'v') → result */
    async cmd(...args) {
...
module.exports = { evaluate, dedupeKey, formatMessage, broadcast, store, CONFIRM_WINDOW };
