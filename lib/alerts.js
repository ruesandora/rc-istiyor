...
function dedupeKey(ev, tf) {
  // Olası: aynı çıpa pivot + tür için tek bildirim (aday kaysa bile tekrar etmez)
  if (ev.kind === 'potential') return `rc:pot:usd:${tf}:${ev.type}:${ev.from.time}`;
  return `rc:div:usd:${tf}:${ev.type}:${ev.from.time}:${ev.to.time}`;
}
...
function formatMessage(ev, tf, siteUrl) {
