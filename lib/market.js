 ...
 *
 * Öncelik: OANDA_API_KEY (spot) → Yahoo COMEX SI=F (anahtarsız; YAHOO=off ile kapatılır)
 *          → TWELVEDATA_API_KEY → Binance XAGUSDT. Hata olursa bir sonrakine geçilir.
...
async function fresh(env) {
  ...
  if (env.YAHOO !== 'off') {
    try {
      return { source: YAHOO.SOURCE, symbol: YAHOO.SYMBOL, timeframes: await YAHOO.loadAll() };
    } catch (e) { warnings.push(e.message); }
  }
