...
async function series(interval, outputsize, key, sym) {
  const url = `${BASE}?symbol=${encodeURIComponent(sym || 'XAG/USD')}&interval=${interval}&outputsize=${outputsize}&timezone=UTC&order=asc&apikey=${encodeURIComponent(key)}`;
...
async function loadAll(key, sym) {
  const [hourly, daily] = await Promise.all([series('1h', 2400, key, sym), series('1day', 1500, key, sym)]);
  return {
    '1h': hourly.slice(-600),
